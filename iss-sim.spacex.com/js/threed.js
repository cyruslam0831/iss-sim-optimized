...
function initButtons() {
    deviceSettings.isMobile
        ? ($("#begin-button").addEventListener("touchstart", hideIntro, !1),
          ...
          $("#settings-button").addEventListener("touchstart", settingsOpen, !1),
        ...
        : ($("#begin-button").addEventListener("click", hideIntro, !1),
          ...
          $("#settings-button").addEventListener("click", settingsOpen, !1),
...
function showInterface() {
    (interfaceAnimationIn = new TimelineMax({
        ...
        interfaceAnimationIn.fromTo("#range .label", 1, { scrambleText: { text: " " } }, { scrambleText: { text: "距離", chars: "0123456789!@#$%^&*()" }, ease: "none" }, 3),
        interfaceAnimationIn.fromTo("#rate .label", 1, { scrambleText: { text: " " } }, { scrambleText: { text: "速率", chars: "0123456789!@#$%^&*()" }, ease: "none" }, 3),
        interfaceAnimationIn.fromTo("#roll-label", 1, { scrambleText: { text: " " } }, { scrambleText: { text: "滾轉", chars: "0123456789!@#$%^&*()" }, ease: "none" }, 3.25),
        interfaceAnimationIn.fromTo("#pitch-label", 1, { scrambleText: { text: " " } }, { scrambleText: { text: "俯仰", chars: "0123456789!@#$%^&*()" }, ease: "none" }, 3.25),
        interfaceAnimationIn.fromTo("#pyr-label", 1, { scrambleText: { text: " " } }, { scrambleText: { text: "方位", chars: "0123456789!@#$%^&*()" }, ease: "none" }, 3.25),
        interfaceAnimationIn.fromTo("#yaw-label", 1, { scrambleText: { text: " " } }, { scrambleText: { text: "偏航", chars: "0123456789!@#$%^&*()" }, ease: "none" }, 3.25),
...
function startTimer() {
    ...
    var countDownDate = new Date().getTime() + timeLimit + 8000;
        var x = setInterval(function() {
            ...
            // Update the timer element with the formatted display
            document.getElementById("timer").innerHTML = timerDisplay;
        
            // If the countdown is over, clear the timer interval
            if (distance < 0 || !gameStarted) {
            clearInterval(x);
            document.getElementById("timer").innerHTML = "";
            //console.log("stopping timer");
            if (distance < 0) {
                $("#fail-message").innerHTML = "時間已到!";
                hideInterface("fail");
            }
            }
        }, 10);
...
function hideInterface(t) {
    (isGameOver = !0),
        ...
        "success" === t &&
            ...
            min = Math.floor((finish - start) / (1000 * 60));
            sec = Math.floor(((finish - start) / 1000) % 60);
            ($("#success-message").innerHTML = "時間: " + min.toString().padStart(2, "0") + ":" + sec.toString().padStart(2, "0"))
        "fail" === t &&
...
function render() {
    ...
    if ((scene.updateMatrixWorld(), isWarpComplete)) {
        ...
        //console.log(`${(motionVector.x).toFixed(3)}, ${(motionVector.y).toFixed(3)}, ${(motionVector.z).toFixed(3)}` );
            ...
            (currentRotationZ = currentRotationZ += (0.001 * -targetRotationZ - currentRotationZ) * moveSpeed),
        ...
        if ((($("#range .rate").innerHTML = n.toFixed(1) + " m"), (rangeRateCounter += 1), 20 <= rangeRateCounter)) {
            ...
            (rateCurrent = 0.75 * (1e3 * ((n - prevRange) / (s - prevRangeTime)))),
                ...
                ($("#rate .rate").innerHTML = rateCurrent.toFixed(3) + " m/s"),
                (prevRange = n),
                (prevRangeTime = s);
        }
...
function checkCollision() {
    if (!0 !== isGameOver) {
        ...
        if (
            ...
            0 < t.length && (($("#fail-message").innerHTML = "您撞到了國際太空站。"), hideInterface("fail")),
            500 < e && (($("#fail-message").innerHTML = "您距離國際太空站太遠了。"), hideInterface("fail")),
            ...
            if (o <= toleranceRotation && a <= toleranceRotation && r <= toleranceRotation && i <= toleranceRate && n <= toleranceRate && s <= toleranceRate) hideInterface("success");
                ...
                var l = "";
                (o > toleranceRotation || a > toleranceRotation || r > toleranceRotation) && (l = "轉動"),
                    (i > toleranceRate || n > toleranceRate || s > toleranceRate) && ("" != l && (l += ", "), (l += "速度")),
                    ($("#fail-message").innerHTML = "以下數值出現偏差: <span class='red'>" + l + "<span>"),
                    hideInterface("fail");
            }
            /*if (timeLimit != -1 && distance < 0) {
                $("#fail-message").innerHTML = "Time is up!";
                hideInterface("fail");
        }*/
    }
}
...
function toggleGravity() {
    isGravity
        ? ((isGravity = !1), $("#setting-gravity").classList.remove("active"), ($("#setting-gravity span").innerHTML = "關"))
        : ((isGravity = !0), $("#setting-gravity").classList.add("active"), ($("#setting-gravity span").innerHTML = "開"));
}
function toggleEarthShape() {
    isFlatEarthCreated || createFlatEarth(),
        isFlatEarth
            ? ((isFlatEarth = !1), (earthObject.visible = !0), (flatEarthObject.visible = !1), ($("#setting-earth span").innerHTML = "球體"))
            : ((isFlatEarth = !0), (earthObject.visible = !1), (flatEarthObject.visible = !0), ($("#setting-earth span").innerHTML = "平面"));
}
function toggleTimer() {
    if (timeLimit === -1) {
        timeLimit = 60000;
        $("#setting-timer span").innerHTML = "1分鐘"
    } else if (timeLimit === 60000) {
        timeLimit = 120000;
        $("#setting-timer span").innerHTML = "2分鐘"
    } else if (timeLimit === 120000) {
        timeLimit = 180000;
        $("#setting-timer span").innerHTML = "3分鐘"
    } else if (timeLimit === 180000) {
        timeLimit = 300000;
        $("#setting-timer span").innerHTML = "5分鐘"
    } else {
        timeLimit = -1;
        $("#setting-timer span").innerHTML = "無"
    }
}
/*
Difficulty list:
1: Easy
Tolerance rate: 0.008
Tolerance rotation: 0.6

2: Normal
Tolerance rate: 0.006
Tolerance rotation: 0.4

3: Hard
Tolerance rate: 0.004
Tolerance rotation: 0.2
...
function toggleDifficulty() {
    if (difficulty == 1) {
        ...
        toleranceRate = 0.006
        toleranceRotation = 0.4
        $("#setting-difficulty span").innerHTML = ("普通")
    } else if (difficulty == 2) {
        difficulty = 3
        toleranceRate = 0.004
        toleranceRotation = 0.2
        $("#setting-difficulty span").innerHTML = ("困難")
    } else {
        difficulty = 1
        toleranceRate = 0.008
        toleranceRotation = 0.6
        $("#setting-difficulty span").innerHTML = ("簡單")
    }
}
...
window.addEventListener("gamepadconnected", (e) => {
    control = navigator.getGamepads()[e.gamepad.index];
    console.log(
      "Gamepad connected at index %d: %s. %d buttons, %d axes.",
      control.index,
      control.id,
      control.buttons.length,
      control.axes.length,
    );
    gamePads[gamePadCount] = e.gamepad.index
    gamePadCount += 1
  })
...
function switchController() {
    if (gamePadCount == 2) {
        ...
        gamePads[0] = gamePads[1]
        gamePads[1] = temp
        console.log("Controllers switched")
    } else {
        console.log("[ERROR!] Incorrect number of controllers")
    }
}
  
  /*
  Axes:
  0: X-axis (-1 Leftward +1 Rightward)
  1: Y-axis (-1 Forward +1 Backward)
  2
  3
  4
  5: Twist (-1 Left +1 Right)
  6: Throttle (-1 - +1 +)
  7
  8
  9: POV1
